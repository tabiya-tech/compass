...
### Step 1.4: Create Identity Project Folders.
...
### Step 1.8: Set up the Realm Stack.
...
**Create a new project** in the [Google Cloud Console](https://console.cloud.google.com/cloud-resource-manager) under the appropriate [identity folder created in step 1.4](#step-14-create-identity-project-folders) based on the environment type.
...
**Configure Branding**:
- Upload an **App Logo**.
- Add the **authorized domain** (`<base-domain-name>`) from the [Realm stack](#step-18-set-up-the-realm-stack).
...
When executing the build and upload scripts, first authenticate to the service account using the [step 4.0](#step-40-authenticate-with-the-service-account).
...
### Step 4.0: Authenticate with the Service Account and AWS Profile.

Before deploying the environment, authenticate with the service account using the following command:

```shell
gcloud auth activate-service-account --key-file=<ENV_TYPE_GOOGLE_APPLICATION_CREDENTIALS>
export GOOGLE_APPLICATION_CREDENTIALS=<ENV_TYPE_GOOGLE_APPLICATION_CREDENTIALS>
```

Replace `<ENV_TYPE_GOOGLE_APPLICATION_CREDENTIALS>` with the actual path to your service account key file. If the environment is a lower environment, use the lower service account key file. If it is an upper environment, use the upper service account key file.

Also, you have to Authenticate with the `AWS_PROFILE` which has access to the AWS account where the name servers are hosted.
...
Refer to [Step 1.9](#step-19-create-and-upload-the-stack-config-yaml) for the correct format and usage.
...
- **For the stack configuration YAML file**, refer to the [stack config template](/iac/templates/stack_config.template.yml).
...
### Step 4.10: Configure the Auth Domains.
