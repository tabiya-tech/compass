...
const YesNoQuestion: React.FC<YesNoQuestionProps> = ({ questionText, questionId, ratingValue, notifyChange, comments, showCommentsOn }) => {
  // function to determine if comments should be shown
  const shouldShowComments = useCallback((value: boolean | null) =>
    (showCommentsOn === YesNoEnum.Yes && value === true) ||
    (showCommentsOn === YesNoEnum.No && value === false), [showCommentsOn]);
  ...
  return (
    <FormControl component="fieldset" fullWidth margin="normal"
                 sx={{ display: "flex", flexDirection: "column", gap: 1, margin: 0 }} data-testid={DATA_TEST_ID.FORM_CONTROL} >
      ...
      <RadioGroup aria-label={questionId} name={questionId} value={ratingValue === null ? "" : ratingValue} onChange={handleChange} row>
