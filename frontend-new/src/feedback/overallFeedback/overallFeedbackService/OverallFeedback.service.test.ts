...
import { QuestionsConfig, QuestionType } from "src/feedback/overallFeedback/feedbackForm/feedbackForm.types";
...
describe("OverallFeedbackService", () => {
  ...
  describe("getQuestionsConfig", () => {
    test("should fetch questions config successfully", async () => {
      ...
      setupAPIServiceSpy(StatusCodes.OK, givenQuestionsConfig, "application/json;charset=UTF-8");
      ...
      expect(global.fetch).toHaveBeenCalledWith(`${givenApiServerUrl}/conversations/${givenSessionId}/feedback/questions`, expect.objectContaining({
        expectedContentType: "application/json",
        expectedStatusCode: 200,
        headers: { map: { authorization: "Bearer foo token", "content-type": "application/json" } },
        method: "GET",
      }));
