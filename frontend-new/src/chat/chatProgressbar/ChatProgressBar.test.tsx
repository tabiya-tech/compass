...
import ChatProgressBar, { DATA_TEST_IDS } from "./ChatProgressBar";
...
describe("ChatProgresBar", () => {
  describe("render tests", () => {
    it("should render the progress bar with the correct phase and percentage", async () => {
      ...
      const container = screen.getByTestId(DATA_TEST_IDS.CONTAINER);

      // AND the progress bar text should be in the document
      const progressBar = screen.getByTestId(DATA_TEST_IDS.PROGRESS_BAR_PHASE_TEXT);
      ...
      const progressBarLabel = screen.getByTestId(DATA_TEST_IDS.PROGRESS_BAR_LABEL);
      ...
      const progressBarElement = screen.getByTestId(DATA_TEST_IDS.PROGRESS_BAR);
