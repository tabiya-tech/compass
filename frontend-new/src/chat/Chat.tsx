...
const Chat: React.FC<ChatProps> = ({ showInactiveSessionAlert = false, disableInactivityCheck = false }) => {
  ...
  const handleOpenExperiencesDrawer = useCallback(async () => {
    ...
    try {
      const experienceService = ExperienceService.getInstance(activeSessionId);
  ...
  const initializeChat = useCallback(
    async (user_id: string | null, currentSessionId: number | null) => {
      ...
      try {
        ...
        console.log("history.experiences_explored", history.experiences_explored);
  ...
  useEffect(
    () => {
      if (initializationRef.current) {
        console.debug("initialize Once Effect skipped");
        return;
      }
