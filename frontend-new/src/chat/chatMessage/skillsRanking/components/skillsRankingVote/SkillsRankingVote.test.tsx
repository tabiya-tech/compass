...
describe("SkillsRankingVote tests", () => {
  describe("render tests", () => {
    test("should render component successfully", () => {
      ...
      // AND expect the rank icons to be in the document
      const rankIcons = screen.getAllByTestId(DATA_TEST_ID.SKILLS_RANKING_VOTE_RADIO);
      expect(rankIcons).toHaveLength(RANK_OPTIONS.length);
      // AND expect the divider to be in the document
      const divider = screen.getAllByTestId(DATA_TEST_ID.SKILLS_RANKING_VOTE_DIVIDER);
      expect(divider).toHaveLength(RANK_OPTIONS.length - 1);
  ...
  describe("action tests", () => {
    test("should call onRankSelect when a rank icon is clicked", async () => {
      ...
      const rankIcon = screen.getAllByTestId(DATA_TEST_ID.SKILLS_RANKING_VOTE_RADIO)[0];
