...
describe("AuthenticationStateService", () => {
  ...
  describe("AuthenticationStateService methods", () => {
    ...
    describe("getToken", () => {
      it("should return null when no token is set and no token in storage", () => {
        // GIVEN the service is newly instantiated
        service = AuthenticationStateService.getInstance();
        // AND no token in storage
      ...
      it("should return token from state when set", () => {
        ...
        const givenToken = "mock-token-123";
        service.setToken(givenToken);
        // AND the persistent storage service
        ...
        // AND expect the persistent storage service not to have been called
        expect(PersistentStorageService.getToken).not.toHaveBeenCalled();
      });

      it("should get token from storage when not in state", () => {
        // GIVEN the service is newly instantiated
        service = new (AuthenticationStateService as any)();
        // AND a token in storage
        const givenToken = "mock-token-123";
    ...
    describe("setToken", () => {
      it("should set the token and update persistent storage", () => {
        ...
        const result = service.setToken(givenToken);
