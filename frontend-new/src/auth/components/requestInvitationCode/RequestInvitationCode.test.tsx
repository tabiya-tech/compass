...
import RequestInvitationCode, {
  DATA_TEST_ID as REQUEST_INVITATION_CODE_DATA_TEST_ID,
  UI_TEXT,
} from "./RequestInvitationCode";
import { InvitationType } from "src/auth/services/invitationsService/invitations.types";
import RequestInvitationCodeFormModal, {
  DATA_TEST_ID as REQUEST_INVITATION_CODE_FORM_MODAL_DATA_TEST_ID,
...
describe("RequestInvitationCode", () => {
  ...
  test("renders the correct text and link when Sentry is initialized", () => {
    ...
    expect(
      screen.getByTestId(`${REQUEST_INVITATION_CODE_DATA_TEST_ID.REQUEST_INVITATION_CODE_LINK}`)
    ).toBeInTheDocument();
  });
  ...
  test("calls notifyOnModalOpened when the modal is opened", async () => {
    ...
    // WHEN the component is rendered
    render(
      <RequestInvitationCode invitationCodeType={InvitationType.LOGIN} notifyOnModalOpened={mockNotifyOnModalOpened} />
    );
