...
export const customFetch = async (apiUrl: string, init: ExtendedRequestInit = defaultInit): Promise<Response> => {
  ...
  * */
  const refreshToken = async (attempt: number): Promise<string> => {
  ...
  * */
  const checkToken = async (token: string | null, attempt: number) => {
    if (!token) {
      console.warn(
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (authRequired) {
      try {
        await checkToken(token, attempt)
        ...
        if ((e as TokenError).message === TokenValidationFailureCause.TOKEN_EXPIRED) {
          ...
          token = await refreshToken(attempt);
