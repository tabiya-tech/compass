...
jest.mock("src/features/skillsRanking/utils/createMessages", () => ({
  ...
  createDisclosureMessage: jest.fn((_s: unknown, next: unknown) => makeMessage("disclosure", { onContinue: next as MessagePayload["onContinue"] })),
...
describe("useSkillsRanking", () => {
  ...
  test("replays phases and advances through the flow", async () => {
    ...
    // expect(addedMessages.map((m) => m.type)).toContain(["prompt", "brief", "effort", "disclosure"]);
