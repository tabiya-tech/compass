...
// Helper function to get the next phase for a given experiment group and current phase
export const getNextPhaseForGroup = (
  experimentGroup: SkillsRankingExperimentGroups,
  currentPhase: SkillsRankingPhase
): SkillsRankingPhase | null => {
  const flowPath = getFlowPathForGroup(experimentGroup);
  const currentIndex = flowPath.indexOf(currentPhase);
  if (currentIndex === -1 || currentIndex === flowPath.length - 1) {
    return null;
  }
