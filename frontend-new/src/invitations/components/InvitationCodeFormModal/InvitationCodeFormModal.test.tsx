...
import InvitationCodeFormModal, { DATA_TEST_ID } from './InvitationCodeFormModal';
...
describe('InvitationCodeFormModal', () => {
  const setUpMocks = () => {
  ...
  it('renders correctly when modal is shown', () => {
    ...
    render(<InvitationCodeFormModal show={givenShown} onSuccess={jest.fn()} onClose={onClose} />)
  ...
  it('calls onClose when valid registration code is entered', async () => {
    ...
    // WHEN the component is rendered
    render(<InvitationCodeFormModal onSuccess={jest.fn()} show={givenShown} onClose={onClose} />);
    ...
    await waitFor(() => {
      expect(enqueueSnackbar).toHaveBeenCalledWith('validated the registration code', { variant: 'success' });
    })
