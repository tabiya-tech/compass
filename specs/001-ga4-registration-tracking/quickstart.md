...
2) GTM config: Create Data Layer Variable `user_id`; GA4 Config Tag → User-ID = `user_id`; ensure config fires after identity push or refire config when identity is set. Social (Google) and email registration now call `user_identity_set` immediately after preferences creation so GA4 has `user_id` before `registration_complete` fires—keep this ordering for any new auth flows.
