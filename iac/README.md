...
- Create a GCP project which will be root project. It will be used to manage the resources of other target projects.
...
> **Why?:** Service Usage API is used for enabling APIs
...
the reason why this is done manually is that there are some manual steps that need to be done in the Identity Platform console like setting up consent screen.
...
We need to have a service account created on root project. This service account will be used by pulumi to create resources, enable and re-enable other APIs.
Before running this iac project you need service account.
...
- it must be able to manage groups on the workspace (Note: this is done on admin organisation as groups are only created on google workspace admin panel)
  - create a group
  - delete a group
  - update a group
  - get a group
> Note: it is recommended to create a custom admin role named  `Groups Small Admin` with only CRUD permissions on groups.
- it must be a member of the `root project` in order for it to use ServiceUsageAPI so that it can be able to enable services on other projects/environments.
- This service account is used to run pulumi must be granted access to the root project.
...
See the details in the [Service Account](#service-account) for the required roles of the principal. 
...
- [organization](organization): sets up the organization specific configuration.
- [environment](environment): sets up the project/environment specific configuration.
