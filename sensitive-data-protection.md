...
#### 1. You need to create a certificate (an RSA private/public key).**

1. how to generate a certificate using openssl
...
_You can use alternative methods like `ssh-kegen` or `GnuPG` to generate the keys, among others._

#### 2. For encrypting, you need to set up the frontend

You are going to add your public key found at `public-key.pem` to the frontend. Because it is possible to use different keys eg: key rotation, 
you are going to need to add the key ID. This is a unique identifier for the key.

Refer to this [frontend documentation](frontend-new/README.md#installation) for more information.

#### 3. For decrypting, you need to use the decrypt sensitive data script.

We have a script that will help you decrypt the data.
The [script](backend/scripts/sensitive_data/decrypt_sensitive_personal_data.py) is located in the backend.
You are going to need the `private-key.pem` path, the password you used to generate the private key and the output path.

[//]: # (TODO: Fix the script to handle multiple key ids)
