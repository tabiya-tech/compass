...
class TestOrderEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_post_creates_order_successfully(self, client):
        ...
    
    @pytest.mark.asyncio
    async def test_post_returns_bad_request_when_order_cannot_be_created(self, client):
        ...
```
...
// AND that the user can successfully login 
// WHEN the user provides their username 
// AND the password 
// AND clicks on the "Login" button
// THEN expect the user to be successfully logged in
```
...
given_username = "testuser"
# AND a valid password
given_password = "securepassword123"
# AND the authentication service will successfully authenticate
setup_mock_auth_service(will_succeed=True)

# WHEN the login endpoint is called with the given credentials
actual_response = await client.post("/auth/login", json={
...
# AND the response body to contain a valid token
assert actual_response.json()["token"] is not None
# AND the username in the response to match the given username
assert actual_response.json()["username"] == given_username
```
...
```python
# WHEN the foo function is called
actual_foo_result = foo()
# AND the bar function is called
actual_bar_promise = bar()
```
...
```python
# THEN the expected foo value
expected_foo_outcome = "foo"
# AND the expected bar value
expected_bar_outcome = "bar"
```
...
describe("test login() function", () => {
  ...
  it("should return status UNAUTHORIZED on failed login", async () => {
    // GIVEN a username
    // AND a password
    // AND the user will fail to login
