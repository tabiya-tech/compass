...
async def populate_sample_conversation(*, mongo_uri: str, database_name: str, session_id: int, hot_run: bool = False):
    """
    ...
    Args:
        ...
        session_id: Session ID to use (required)
    ...
    try:
        ...
        logger.info(f"Starting sample conversation population {'(HOT RUN)' if hot_run else '(DRY RUN)'} for session ID: {session_id}")
        ...
        try:
            existing_state = await state_store.get_state(session_id)
            if not existing_state:
                errored += 1
                raise ValueError(f"Session ID {session_id} does not exist in the database. Please create the session first.")
            logger.info(f"Found existing session with ID {session_id}. Proceeding with population.")
        ...
        agent_director_state = create_agent_director_state(session_id)
        explore_experiences_director_state = create_explore_experiences_director_state(session_id)
        conversation_memory_manager_state = create_conversation_memory_manager_state(session_id, conversation_history)
        collect_experience_state = create_collect_experience_state(session_id)
        skills_explorer_agent_state = create_skills_explorer_agent_state(session_id)
        ...
        logger.info(f"Sample conversation created for existing session ID: {session_id}")
        ...
        if hot_run:
            try:
                ...
                processed_states += 1
                logger.info(f"Successfully populated sample conversation to database for session ID: {session_id}")
            except Exception as err:
                errored += 1
                logger.error(f"Error saving state for session {session_id}: {str(err)}")
                raise
        else:
            processed_states += 1
...
def create_agent_director_state(session_id: int) -> AgentDirectorState:
...
def create_explore_experiences_director_state(session_id: int) -> ExploreExperiencesAgentDirectorState:
    ...
    experience_states[freelance_experience.uuid] = ExperienceState(
        dive_in_phase=DiveInPhase.PROCESSED,
...
def create_collect_experience_state(session_id: int) -> CollectExperiencesAgentState:
...
def create_skills_explorer_agent_state(session_id: int) -> SkillsExplorerAgentState:
    ...
    return SkillsExplorerAgentState(
        session_id=session_id,
        first_time_for_experience={uuid: False for uuid in experience_uuids},
        experiences_explored=experience_uuids
    )


def create_conversation_memory_manager_state(session_id: int, conversation_history: ConversationHistory) -> ConversationMemoryManagerState:
    ...
    return ConversationMemoryManagerState(
        ...
        summary="The user discussed their work experience as a Baker at Sweet Delights Bakery from 2019 to 2021, where they prepared various breads, pastries, and desserts. They also shared their experience as a Freelance Cake Designer since 2022, where they design custom cakes for special events and manage client relationships."
    )
...
def create_sample_conversation_history() -> ConversationHistory:
    ...
    conversation_turns = [
        ...
        ConversationTurn(
            ...
            output=AgentOutput(
                message_id=str(ObjectId()),
                message_for_user="Okay, let's start by getting a general overview of your work experiences. This is just a quick snapshot to begin with - we'll dive deeper into each experience later.\n\nHave you ever worked for a company or someone else's business for money?",
                finished=False,
        ...
        ConversationTurn(
            ...
            output=AgentOutput(
                message_id=str(ObjectId()),
                message_for_user="Got it. So, you were a baker at Sweet Delights Bakery in Cape Town. You started in January 2019 and worked until December 2021. It was a paid job.\n\nWould you like to add or change anything about this experience before we move on?",
                finished=False,
                agent_type=AgentType.COLLECT_EXPERIENCES_AGENT,
        ...
        ConversationTurn(
            ...
            input=AgentInput(
                message_id=str(ObjectId()),
                message="Yes, as I mentioned, I'm a freelance cake designer. I design and create custom cakes for special events and weddings, manage client consultations, and run social media marketing for my business.",
                sent_at=datetime.now(timezone.utc).replace(minute=6, second=0, microsecond=0)
            ),
            output=AgentOutput(
                message_id=str(ObjectId()),
                message_for_user="Let's recap the information we have collected so far:\n\n• Baker (Waged Employment), 2019/01 - 2021/12, Sweet Delights Bakery, Cape Town\n• Freelance Cake Designer (Self-Employment), 2022/01 - Present, Self-employed, Johannesburg\n\nIs there anything you would like to add or change? If one of the experiences seems to be duplicated, you can ask me to remove it.",
                finished=False,
                agent_type=AgentType.COLLECT_EXPERIENCES_AGENT,
        ...
        ConversationTurn(
            ...
            output=AgentOutput(
                message_id=str(ObjectId()),
                message_for_user="Okay, cool. So, we're going to talk about your experience working as a baker.\n\nI'm going to ask you some questions about your time working as a baker. Try to be as descriptive as possible in your answers. The more you tell me about your experience, the better we can understand it.\n\nSo, tell me, what was a typical day like at work?",
                finished=False,
        ...
        ConversationTurn(
            ...
            input=AgentInput(
                message_id=str(ObjectId()),
                message="I prepared a variety of breads, pastries, and desserts daily. I also collaborated with kitchen staff, implemented quality control procedures, and mentored junior bakers.",
                sent_at=datetime.now(timezone.utc).replace(minute=8, second=0, microsecond=0)
            ),
            output=AgentOutput(
                message_id=str(ObjectId()),
                message_for_user="That's impressive! You've gained valuable skills in baking with bread and pastry making, as well as soft skills like collaboration and mentoring. I'd like to know more about your freelance cake design work. What kind of cakes do you specialize in?",
                finished=False,
                agent_type=AgentType.EXPLORE_SKILLS_AGENT,
                agent_response_time_in_sec=1.0,
                llm_stats=[],
        ...
        ConversationTurn(
            ...
            input=AgentInput(
                message_id=str(ObjectId()),
                message="I specialize in fondant work and sugar craft for most of my clients. For client management, I have detailed consultations and make sure to set clear expectations about designs, flavors, and delivery timelines.",
        ...
        ConversationTurn(
            ...
            output=AgentOutput(
                message_id=str(ObjectId()),
                message_for_user="It was great exploring your skills with you! I've learned a lot about your experience as a baker and as a freelance cake designer. Goodbye!",
                finished=True,
