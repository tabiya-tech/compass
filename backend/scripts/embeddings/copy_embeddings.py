...
class CopyEmbeddingsScriptSettings(BaseSettings):
    ...
    """ The URI of the target MongoDB we will be copying the data to."""

    target_db_name: str
    """ The name of the target database to store the embeddings."""
...
async def main():
    parser = argparse.ArgumentParser(dedent("""
                                        ...
                                        You can optionally delete existing documents and regenerate indexes post-copy.
                                        ...
                                        Required environment variables:
                                          ...
                                          - COPY_EMBEDDINGS_SCRIPT_TARGET_MONGODB_URI: MongoDB URI of the target database
                                          - COPY_EMBEDDINGS_SCRIPT_TARGET_DB_NAME: Name of the target database
                                        ...
                                        Example:
                                          ...
                                          COPY_EMBEDDINGS_SCRIPT_TARGET_MONGODB_URI=mongodb://localhost:27018 \\
                                          COPY_EMBEDDINGS_SCRIPT_TARGET_DB_NAME=target_db \\
    ...
    target_client = AsyncIOMotorClient(settings.target_mongodb_uri)
    logger.info(f"Target MongoDB URI: {redact_credentials_from_uri(settings.target_mongodb_uri)}")
    target_db = target_client.get_database(settings.target_db_name)
    logger.info(f"Target database: {settings.target_db_name}")

    total = len(CompassEmbeddingsCollections) + 1
    progress = tqdm(
        desc="Processing",
        total=total,
    )
    ...
    async with await source_client.start_session() as source_session:
        for index, col in enumerate(CompassEmbeddingsCollections):
    ...
    if args.generate_indexes:
        await generate_indexes(hot_run=args.hot_run,
                               db=target_db,
                               logger=logger)
    progress.update(1)
