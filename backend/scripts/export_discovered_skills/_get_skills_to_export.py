...
async def _get_skills_to_export(*,
    ...
    for conversation in conversations:
        conversation = AgentDirectorState.from_document(conversation)
        session_ids.append(conversation.session_id)
    ...
    for conversation in conversations:
        conversation = AgentDirectorState.from_document(conversation)
        explore_experiences_state = explore_experiences_states_map.get(conversation.session_id)

        if explore_experiences_state is None:
            logger.warning("No explore experiences state found for session ID: %s", conversation.session_id)
            break
        ...
        for experience in explore_experiences_state.experiences_state.values():
            for skill in experience.experience.top_skills:
                ...
                exportable_skills.append(dict(
                    ...
                    conversation_phase=conversation.current_phase.name,
