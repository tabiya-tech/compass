...
- `BACKEND_SENTRY_CONFIG`: (optional) A JSON object controlling backend Sentry behavior. Supported fields:
  ...
  - `enableLogs` (boolean): When false, only error events are sent regardless of `levels`
  - `levels` (string[]): One or more of `debug`, `info`, `warning`, `error` determining the minimum log level captured as Sentry events when `enableLogs` is true
