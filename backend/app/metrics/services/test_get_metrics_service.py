...
class TestGetMetricsService:
    ...
    async def test_get_metrics_service_concurrent_calls(self, mocker: pytest_mock.MockFixture, setup_application_config: ApplicationConfig):
        ...
        metrics_service_instance_1, metrics_service_instance_2 = await asyncio.gather(
            get_metrics_service(application_db=_in_memory_metrics_database, app_config=setup_application_config, user_preference_repository=_mock_user_preference_repository),
            get_metrics_service(application_db=_in_memory_metrics_database, app_config=setup_application_config, user_preference_repository=_mock_user_preference_repository)
        )
        ...
        assert metrics_service_instance_1.enable_metrics == setup_application_config.enable_metrics  # type: ignore
    ...
    async def test_get_metrics_service_subsequent_calls(self, mocker: pytest_mock.MockFixture, setup_application_config: ApplicationConfig):
        # GIVEN random in-memory metrics database
        _in_memory_metrics_database = mocker.MagicMock()
        ...
        metrics_service_instance_1 = await get_metrics_service(
            application_db=_in_memory_metrics_database, app_config=setup_application_config, user_preference_repository=_mock_user_preference_repository)
