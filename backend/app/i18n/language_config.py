...
class LanguageConfig(BaseModel):
    ...
    def is_locale_available(self, locale: Locale) -> bool:
        """Whether the given locale is one of the configured available_locales."""
        return any(entry.locale == locale for entry in self.available_locales)
    ...
    def basic_locale_validation(cfg: "LanguageConfig", name: str, locale: Locale):
        ...
        if not cfg.is_locale_available(locale):
            raise ValueError(f"{name} {locale.value} must be in available_locales")
