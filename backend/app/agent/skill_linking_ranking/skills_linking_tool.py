...
class SkillsLinkingTool:
    ...
    async def link_and_rank_skills(self, *,
        ...
        # 2.2 Count the number of occurrences of each skill and update the skill_stats dictionary
        # 3. Return the top_k most frequently occurring skills
        """
        # Prepare the data
        skill_stats: dict[str, SkillStat] = {}  # The uuid is the key
        ...
        # 2. For each responsibility (embedding),
        for responsibility_embedding in responsibilities_embeddings:
            ...
            for skill in similar_skills:
                if skill.UUID in skill_stats:
                    ...
                    skill_stats[skill.UUID] = SkillStat(skill=skill, count=1, score_sum=skill.score)
        # 3. Return the top_k most frequently occurring skills
        ...
        return [skill_stat.skill for skill_stat in sorted_skills_score[:top_k]]
