...
class _ContextualizationLLM:
    ...
    async def _internal_execute(
        ...
        unexpected_exception_penalty_level = 4
        no_llm_output_penalty_level = 3
        contextual_titles_missing_penalty_level = 2
        contextual_titles_mismatch_penalty_level = 1
        ...
        try:
            ...
            if not llm_response:
                ...
                return ContextualizationLLMResponse(
                    ...
                    llm_stats=llm_stats
                ), penalty, ValueError("LLM did not return any output")
            ...
            if len(contextual_titles) == 0:
                _result_penalty = get_penalty(contextual_titles_missing_penalty_level)
                _raised_error = ValueError("The LLM did not returned empty contextual titles")
                ...
                _result_penalty = get_penalty_for_multiple_errors(level=contextual_titles_mismatch_penalty_level,
            ...
            # If any unexpected exception occurs, we log it and return the input title.
            self._logger.error("Unexpected error while calling the LLM: %s", e, exc_info=True)
            penalty = get_penalty(unexpected_exception_penalty_level)
            return ContextualizationLLMResponse(
                contextual_titles=[experience_title],
                llm_stats=[]
            ), penalty, e
