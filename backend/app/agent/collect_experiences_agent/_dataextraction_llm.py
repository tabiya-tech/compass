...
from app.agent.collect_experiences_agent.data_extraction_llm._entity_extraction_tool import EntityExtractionTool
from app.agent.collect_experiences_agent.data_extraction_llm._intent_analyzer_tool import IntentAnalyzerTool
...
class _DataExtractionLLM:
    def __init__(self, logger: logging.Logger):
        ...
        self._data_processor = ExperienceDataProcessor(logger)
    ...
    async def execute(self,
        ...
        int, list[LLMStats]]:  # REVIEW: Review the return type. why int if it can update many experiences.
        """
        ...
        for turn in context.history.turns:
            conversation_history.append((turn.input.message, turn.output.message_for_user))
        ...
        for command in commands:
            if command.data_operation.lower() == DataOperation.DELETE.value.lower():
                ...
                existing_experience = list(filter(lambda exp: exp.index == command.index, collected_experience_data_so_far))[0] or {}
        ...
        self.logger.debug(context.model_dump_json(indent=2))
        self.logger.debug(f"Operations= {operations}")

        result, _ = self._data_processor.process_experience_operations(
            operations,
            collected_experience_data_so_far=collected_experience_data_so_far,
            current_turn_index=len(context.all_history.turns))
