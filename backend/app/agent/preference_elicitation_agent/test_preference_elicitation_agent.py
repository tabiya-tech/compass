...
class TestPreferenceVector:
    ...
    def test_preference_vector_creation(self):
        ...
        pv = PreferenceVector()
        assert pv.financial_importance == 0.5
        assert pv.job_security_importance == 0.5
    ...
    def test_preference_vector_with_custom_values(self):
        ...
        assert pv.financial_importance == 0.8
        assert pv.work_environment_importance == 0.6
...
@pytest.mark.skip(reason="DB6 youth database contracts are not shipped in core compass")
...
class TestDB6ClientStub:
    ...
    async def test_update_existing_profile(self):
        ...
        profile.preference_vector = PreferenceVector(financial_importance=0.8)
        ...
        assert actual_profile.preference_vector.financial_importance == 0.8
...
class TestAgentDB6Integration:
    ...
    async def test_save_preference_vector_to_db6(self):
        ...
        state = PreferenceElicitationAgentState(session_id=123)
        state.preference_vector.financial_importance = 0.9
        ...
        assert profile.preference_vector.financial_importance == 0.9
    ...
    async def test_save_preference_vector_updates_existing_profile(self):
        ...
        state = PreferenceElicitationAgentState(session_id=123)
        state.preference_vector.financial_importance = 0.7
        agent.set_state(state)
        ...
        assert profile.preference_vector.financial_importance == 0.7  # Added
    ...
    async def test_save_preference_vector_without_db6_client(self):
        ...
        # AND state with preferences
        state = PreferenceElicitationAgentState(session_id=123)
        state.preference_vector.financial_importance = 0.8
        agent.set_state(state)
