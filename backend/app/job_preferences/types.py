...
class WorkLocationType(str, Enum):
    """Explicit work-location constraint requested by the user."""
    REMOTE = "Remote"
    ONSITE = "OnSite"
    HYBRID = "Hybrid"
...
class ContractType(str, Enum):
    """Explicit employment contract type requested by the user."""
    FULL_TIME = "Full-Time"
    PART_TIME = "Part-Time"
    GIG = "Gig"
...
class JobPreferences(BaseModel):
    """
    Job preferences for a user session from preference elicitation.

    Stores BOTH:
    1. Soft preferences (important scores from Bayesian vignette modeling)
    ...
    class Config:
        """Pydantic config: reject unknown fields and serialize enums by value."""
        extra = "forbid"
        use_enum_values = True

    @field_serializer("last_updated")
    def serialize_last_updated(self, last_updated: datetime) -> str:
        """Serialize last_updated as an ISO-8601 string (always UTC)."""
        return last_updated.isoformat()

    @field_validator("last_updated", mode='before')
    def deserialize_last_updated(cls, value: str | datetime) -> datetime:  # pylint: disable=no-self-argument
        """Parse ISO-8601 / datetime into a UTC-aware datetime."""
