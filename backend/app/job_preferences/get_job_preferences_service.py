"""Dependency provider for the singleton JobPreferencesService."""
import asyncio
...
async def get_job_preferences_service(
    ...
    """Return the process-wide JobPreferencesService, instantiating it on first use."""
    global _job_preferences_service_singleton  # pylint: disable=global-statement

    if _job_preferences_service_singleton is None:
        async with _job_preferences_service_lock:
            if _job_preferences_service_singleton is None:
                _job_preferences_service_singleton = JobPreferencesService(
                    repository=JobPreferencesRepository(application_db)
                )
