...
def set_user_conversation_language(user_preferences: UserPreferences) -> str:
    ...
    # The requested locale must be one of the configured available_locales; otherwise we have no
    # translations/date formats for it, so we fall back to the configured conversation fallback locale.
    fallback_locale = app_config.language_config.conversation_fallback_locale
    try:
        conversation_locale = Locale.from_locale_str(user_preferences.language)
        if not app_config.language_config.is_locale_available(conversation_locale):
            logger.info(
                "user preferences language %s is not in available_locales, using fallback %s",
                user_preferences.language, fallback_locale.value
            )
            conversation_locale = fallback_locale
    except ValueError:
        # from_locale_str raises ValueError for None or an unrecognised locale string
        logger.warning(
            "user preferences language %s is not a recognised locale, using fallback %s",
            user_preferences.language, fallback_locale.value
        )
        conversation_locale = fallback_locale
