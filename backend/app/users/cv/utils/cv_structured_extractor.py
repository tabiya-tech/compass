...
class CVStructuredExperienceExtractor:
    ...
    async def extract_structured_experiences(self, markdown_cv: str) -> CVStructuredExtraction:
        ...
        for i, experience in enumerate(structured_experiences):
            ...
            self._logger.debug(
                "Extracted dates for '%s': start_date=%s, end_date=%s",
            ...
            self._logger.info(
                "ExperienceEntity built {title=%s, responsibilities=%d, timeline=%s}",
                experience.experience_title,
                len(responsibilities_data.responsibilities),
                f"start={experience_entity.timeline.start if experience_entity.timeline else None}, end={experience_entity.timeline.end if experience_entity.timeline else None}"
            )
...
- Include start_date and end_date if they appear in the CV
- If end date says "Present", "Current", "Ongoing", or similar: use "Present" as end_date
...
"""
    ...
    def _create_experience_entity(self, experience: CVStructuredExperience, responsibilities_data: ResponsibilitiesData) -> ExperienceEntity:
        ...
        # Create timeline if dates are available (either start or end date)
        timeline = None
