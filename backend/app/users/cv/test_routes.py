...
class TestUploadCV:
    ...
    async def test_success_allowed_mime_type(self, client_with_mocks: TestClientWithMocks, mime: str):
        ...
        given_file = {"file": (f"cv{given_ext}", given_file_content, given_mime)}
        expected_response = {"experiences_data": "parsed"}

        # WHEN uploading the CV
        response = client.post(f"/{given_user_id}/cv", files=given_file)
    ...
    async def test_forbidden_other_user(self, client_with_mocks: TestClientWithMocks, mocker: pytest_mock.MockerFixture):
        ...
        response = client.post(f"/{given_other_user_id}/cv", files=given_file)
    ...
    @pytest.mark.asyncio
    async def test_unsupported_media_type_by_mime(self, client_with_mocks: TestClientWithMocks):
        client, _, mocked_user = client_with_mocks
        # GIVEN a file with an invalid mime and valid extension
        invalid_mime_candidates = [
            "application/octet-stream",
            "application/zip",
            "text/csv",
        ]
        given_invalid_mime = next((m for m in invalid_mime_candidates if m not in ALLOWED_MIME_TYPES), None)
        assert given_invalid_mime is not None
        given_ext = next(iter(ALLOWED_EXTENSIONS))
        given_file_content = b"hello"
        given_file = {"file": (f"cv{given_ext}", given_file_content, given_invalid_mime)}

        # WHEN uploading the CV
        response = client.post(f"/{mocked_user.user_id}/cv", files=given_file)
    ...
    async def test_unsupported_media_type_by_extension(self, client_with_mocks: TestClientWithMocks):
        ...
        given_mime = next(iter(ALLOWED_MIME_TYPES))
        given_file_content = b"hello"
        given_file = {"file": (f"cv{given_invalid_ext}", given_file_content, given_mime)}
    ...
    async def test_payload_too_large(self, client_with_mocks: TestClientWithMocks):
        ...
        given_mime = next(iter(ALLOWED_MIME_TYPES))
        given_ext = next(iter(ALLOWED_EXTENSIONS))
        given_file = {"file": (f"cv{given_ext}", given_oversized_content, given_mime)}
    ...
    @pytest.mark.asyncio
    async def test_missing_file(self, client_with_mocks: TestClientWithMocks):
        client, _, mocked_user = client_with_mocks
        # GIVEN no 'file' part provided
        given_user_id = mocked_user.user_id

        # WHEN uploading the CV without a file
        response = client.post(f"/{given_user_id}/cv", files={})

        # THEN FastAPI returns a validation error
