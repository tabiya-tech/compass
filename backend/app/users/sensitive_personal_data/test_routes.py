...
class TestCreateSensitivePersonalData:
    ...
    async def test_required_data(self, client_with_mocks: TestClientWithMocks, mocker: pytest_mock.MockerFixture):
        ...
        _create_spy = mocker.patch.object(mocked_service, "create")
        _create_spy.side_effect = SensitivePersonalDataRequiredError(given_user_id)
...
class TestSkipSensitivePersonalData:
    @pytest.mark.asyncio
    async def test_skip_success(self, client_with_mocks: TestClientWithMocks, mocker: pytest_mock.MockerFixture):
        ...
        # WHEN a POST request to skip sensitive personal data is made
        _skip_spy = mocker.spy(mocked_service, "skip")
        response = client.post(
            f"/{given_user_id}/sensitive-personal-data/skip"
        )
    ...
    @pytest.mark.asyncio
    async def test_skip_forbidden(self, client_with_mocks: TestClientWithMocks, mocker: pytest_mock.MockerFixture):
        client, mocked_service, mocked_authed_user = client_with_mocks
        ...
        # WHEN a POST request to skip sensitive personal data is made for another user
        _skip_spy = mocker.spy(mocked_service, "skip")
        response = client.post(
            f"/{other_user_id}/sensitive-personal-data/skip"
        )

        # THEN the response is FORBIDDEN
        assert response.status_code == HTTPStatus.FORBIDDEN
        assert response.json() == {"detail": f"User {given_user_id} is not allowed to skip sensitive personal data for another user {other_user_id}"}
        # AND sensitive_personal_data_service's skip method was not called
        _skip_spy.assert_not_called()

    @pytest.mark.asyncio
    async def test_skip_conflict(self, client_with_mocks: TestClientWithMocks, mocker: pytest_mock.MockerFixture):
        client, mocked_service, mocked_authed_user = client_with_mocks
        ...
        _skip_spy.side_effect = DuplicateSensitivePersonalDataError(given_user_id)
    ...
    @pytest.mark.asyncio
    async def test_skip_not_allowed(self, client_with_mocks: TestClientWithMocks, mocker: pytest_mock.MockerFixture):
        client, mocked_service, mocked_authed_user = client_with_mocks

        # GIVEN the user is authenticated
        given_user_id = mocked_authed_user.user_id

        # AND the service raises a bad request error
        _skip_spy = mocker.patch.object(mocked_service, "skip")
    ...
    @pytest.mark.asyncio
    async def test_skip_not_found(self, client_with_mocks: TestClientWithMocks, mocker: pytest_mock.MockerFixture):
        client, mocked_service, mocked_authed_user = client_with_mocks
        ...
        # AND the service raises a not found error
        _skip_spy = mocker.patch.object(mocked_service, "skip")
        _skip_spy.side_effect = UserPreferencesNotFoundError(given_user_id)
    ...
    @pytest.mark.asyncio
    async def test_skip_internal_server_error(self, client_with_mocks: TestClientWithMocks, mocker: pytest_mock.MockerFixture):
        client, mocked_service, mocked_authed_user = client_with_mocks
        ...
        # AND the sensitive personal data service raises an exception
        _skip_spy = mocker.patch.object(mocked_service, "skip")
        _skip_spy.side_effect = Exception("Some error")
