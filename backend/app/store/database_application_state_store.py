...
class DatabaseApplicationStateStore(ApplicationStateStore):
    ...
    async def get_state(self, session_id: int) -> ApplicationState | None:
        ...
        try:
            ...
            # The required collections (everything pre-dating the preference elicitation feature)
            # define whether a session exists. The preference_elicitation_agent_state collection is
            # lazy-initialized — sessions that existed before the feature shipped may legitimately
            # have no row there, in which case we create a default in memory.
            required_results = results[:6]
            preference_elicitation_result = results[6]

            if all(_state_part is None for _state_part in required_results):
                # If all the required states are None, the session does not exist
            ...
            if len(collection_names) != len(required_results):
                self._logger.error(
            ...
            missing_parts = [name for name, result in zip(collection_names, required_results) if result is None]
            if missing_parts:
                self._logger.error(
            ...
            # Successfully retrieved all required states
            (agent_director_state,
             welcome_agent_state,
             explore_experiences_director_state,
             conversation_memory_manager_state,
             collect_experience_state,
             skills_explorer_agent_state) = required_results

            # Lazy-init for preference_elicitation_agent_state: sessions created before the feature
            # shipped won't have this row. Build a default — it will be persisted on the next save_state.
            if preference_elicitation_result is None:
                self._logger.info(
                    "No preference_elicitation_agent_state for session ID %s, creating default (lazy-init)",
                    session_id,
                )
                preference_elicitation_agent_state = PreferenceElicitationAgentState(session_id=session_id)
            else:
                preference_elicitation_agent_state = PreferenceElicitationAgentState.from_document(preference_elicitation_result)
