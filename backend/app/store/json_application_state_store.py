...
class JsonApplicationStateStore(ApplicationStateStore):
    ...
    async def get_state(self, session_id: int) -> ApplicationState | None:
        ...
        try:
            ...
            self._logger.error("Error getting application state from the database: %s", e, exc_info=True)
            return None
    ...
    async def export_state_to_json(self, session_id: int, file_path: str):
    ...
    async def import_state_from_json(self, file_path: str, target_session_id: int, target_user_id: str):
            existing_state = await self.get_state(target_session_id)
            ...
            # Update session_id and user_id in all components
            for component in ['agent_director_state', 'explore_experiences_director_state',
                ...
                if component in state_dict:
                    ...
                    if 'user_id' in state_dict[component]:
                        state_dict[component]['user_id'] = target_user_id
