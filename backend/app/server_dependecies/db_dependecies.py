...
async def initialize_mongo_db():
    ...
    if not INITIALIZED_MONGO_DB:
        ...
        await _mongo_db.get_collection(Collections.EXPLORE_EXPERIENCES_DIRECTOR_STATE).create_index([
            ("session_id", 1)
        ], unique=True)
        ...
        await _mongo_db.get_collection(Collections.SKILLS_EXPLORER_AGENT_STATE).create_index([
            ("session_id", 1)
        ], unique=True)
