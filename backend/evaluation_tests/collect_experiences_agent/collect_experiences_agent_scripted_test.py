...
from evaluation_tests.conversation_libs.utils import _add_turn_to_context, EVALUATION_OUTRO_PROMPT
from evaluation_tests.one_shot_test_case import OneShotTestCase, write_one_shot_test_cases
from evaluation_tests.conversation_libs.evaluators.full_history_evaluator import FullHistoryEvaluator
...
EVALUATION_INTRO_PROMPT = textwrap.dedent("""You are assessing a conversation between a human (SIMULATED_USER) and an
            agent (EVALUATED_AGENT) in charge of collecting past experiences from the user.
""")


test_cases_collect_experiences = write_one_shot_test_cases(FIXED_TURNS, EVALUATION_INTRO_PROMPT) + [OneShotTestCase(
        ...
        evaluator_prompt = textwrap.dedent(f"""{EVALUATION_INTRO_PROMPT}
...
async def test_collect_experiences(test_case):
    ...
    evaluator = FullHistoryEvaluator(evaluation_prompt=test_case.evaluator_prompt)
