...
from evaluation_tests.skill_explore_agent.skill_explore_agent_executors import SkillExploreAgentExecutor, \
    SkillExploreAgentIsFinished, SkillExploreAgentGetConversationContextExecutor

test_cases = [
    ScriptedUserEvaluationTestCase(
        name='user_say_wants_to_finish_early',
        simulated_user_prompt="Scripted user: Just says finish",
        scripted_user=[
            "I want to finish"
        ],
...
async def test_skill_explore_agent_scripted_user(max_iterations: int,
                                                 test_case: ScriptedUserEvaluationTestCase, caplog):
    """
    Conversation test, based on a scripted user.
    Asserts that the skill explore agent is able to complete the conversation.
    ...
    output_folder = os.path.join(os.getcwd(), 'test_output/skill_explore_agent/scripted', test_case.name)
    ...
    execute_evaluated_agent = SkillExploreAgentExecutor(conversation_manager=conversation_manager)
    ...
    config = ConversationTestConfig(
        ...
        is_finished=SkillExploreAgentIsFinished(),
        get_conversation_context=SkillExploreAgentGetConversationContextExecutor(
