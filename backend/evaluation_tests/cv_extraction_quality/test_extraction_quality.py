...
def _list_cv_inputs() -> list[Path]:
    ...
    # pdf_dir = dataset_dir.parent / "cv_extraction_quality" / "test_inputs"
...
async def test_cv_extraction_quality_precision_recall(cv_input_path: Path, common_folder_path: str):
    ...
    # Collect experiences with responsibilities to evaluate
    ...
    for exp in extraction.experience_entities:
        ...
        if responsibilities:
            experiences_with_responsibilities.append((exp, responsibilities))
    
    if not experiences_with_responsibilities:
        pytest.skip(f"No responsibilities extracted from any experience in {cv_input_path.name}. Cannot evaluate precision/recall.")
    
    for exp, responsibilities in experiences_with_responsibilities:
