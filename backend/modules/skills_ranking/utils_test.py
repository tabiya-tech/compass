...
def test_get_possible_next_states():
    ...
    assert next_states == [SkillsRankingCurrentState.EVALUATED, SkillsRankingCurrentState.CANCELLED]
    ...
    # WHEN getting possible next states
    next_states = get_possible_next_states(SkillsRankingCurrentState.CANCELLED)
    # THEN should return empty list
    assert next_states == []
