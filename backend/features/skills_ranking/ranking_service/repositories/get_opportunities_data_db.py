...
async def get_opportunities_data_db(
    ...
    """
    Get the Opportunities Data MongoDB database instance.
    """
    ...
    if _opportunities_data_db_singleton is None:
        ...
        async with _opportunities_data_db_lock:
            ...
            if _opportunities_data_db_singleton is None:
                _logger.info("Connecting to Opportunities Data MongoDB")

                # Create the database instance
                _opportunities_data_db_singleton = get_mongo_db_connection(mongo_db_uri, database_name)

                _logger.info("Connected to Opportunities Data database: %s",
                             await get_database_connection_info(_opportunities_data_db_singleton))
                if not await check_mongo_health(_opportunities_data_db_singleton.client):
                    raise RuntimeError("MongoDB health check failed for Opportunities Data database")

                _logger.info("Successfully pinged Opportunities Data MongoDB")
