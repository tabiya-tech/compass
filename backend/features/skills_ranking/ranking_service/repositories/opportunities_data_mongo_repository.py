...
class OpportunitiesDataRepository(IOpportunitiesDataRepository):
    ...
    async def get_opportunities_skills_uuids(self, limit: int, batch_size: int) -> list[set[str]]:
        try:
            ...
            cursor = self._collection.find({"active": {"$eq": True}}, {'skillGroups.originUUID': 1, '_id': False}).limit(limit).batch_size(
                batch_size)
            ...
            async for doc in cursor:
                ...
                if len(opportunities_docs) >= batch_size:
                    # process the batch
                    skills_sets.extend({skill["originUUID"] for skill in opportunity_doc.get("skillGroups", [])} for opportunity_doc in opportunities_docs)
                    opportunities_docs = []
            ...
            if len(skills_sets) == 0:
                self._logger.error("No active opportunities found in the database.")
