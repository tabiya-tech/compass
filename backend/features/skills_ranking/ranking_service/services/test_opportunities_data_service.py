...
class TestCachingMechanismForOpportunitiesDataService:
    ...
    async def test_sequential_access(self, mocker):
        ...
        first_access_version = opportunities_data_service._last_fetch_time

        second_access = await opportunities_data_service.get_opportunities_skills_uuids()
        second_access_version = opportunities_data_service._last_fetch_time
    ...
    async def test_stale_data_access(self, mocker):
        ...
        assert opportunities_data_service._fetching_task is None

        # AND the _hot_cache is empty
        assert opportunities_data_service._hot_cache is None

        # AND the _cold_cache is empty
        assert opportunities_data_service._cold_cache is None
        ...
        first_last_fetch_time = opportunities_data_service._last_fetch_time
        ...
        assert opportunities_data_service._hot_cache == given_first_db_version

        # GIVEN the data in memory is stale
        #   BY: Simulate stale data by setting last fetch time to a pastime
        opportunities_data_service._last_fetch_time = datetime.now() - timedelta(hours=1)
        ...
        assert opportunities_data_service._fetching_task is not None

        # GIVEN the background task is running and has completed fetching new data
        await opportunities_data_service._fetching_task
        second_last_fetch_time = opportunities_data_service._last_fetch_time
        ...
        assert opportunities_data_service._hot_cache == given_second_db_version
