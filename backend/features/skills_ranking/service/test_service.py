...
class TestSkillsRankingService:
    ...
    async def test_upsert_state_update_valid_transition(
        ...
        # WHEN upserting the state
        service = SkillsRankingService(_mock_skills_ranking_repository, _mock_user_preference_repository)
        given_new_cancelled_after = 50.0
        given_new_perceived_rank = 0.75
        given_new_retyped_rank = 0.85
        result = await service.upsert_state(
            session_id=new_state.session_id,
            phase=new_state.phase,
            cancelled_after=given_new_cancelled_after,
            perceived_rank_percentile=given_new_perceived_rank,
            retyped_rank_percentile=given_new_retyped_rank,
        )
        ...
        _mock_skills_ranking_repository.update.assert_called_once_with(
            ...
            perceived_rank_percentile=given_new_perceived_rank,
            retyped_rank_percentile=given_new_retyped_rank,
            completed_at=None  # Not provided, but the service will set it to None if not provided
        )
