...
class TestGetSkillsRankingService:
    ...
    async def test_get_skills_ranking_service_concurrent_calls(self, mocker: pytest_mock.MockFixture, setup_skills_ranking_feature_config):
        ...
        service_instance_1, service_instance_2 = await asyncio.gather(
            get_skills_ranking_state_service(repository=mock_repository),
            get_skills_ranking_state_service(repository=mock_repository)
        )
    ...
    async def test_get_skills_ranking_service_subsequent_calls(self, mocker: pytest_mock.MockFixture, setup_skills_ranking_feature_config):
        ...
        service_instance_1 = await get_skills_ranking_state_service(repository=mock_repository)
        service_instance_2 = await get_skills_ranking_state_service(repository=mock_repository)
